...
// (accessibility, audio, screen, email, calendar) produce RawEvents that
// flow through RawEventBuffer before AI classification.
...
///
/// RawEvents are value types created by capture services and enqueued in
/// `RawEventBuffer`. They carry enough raw context for the AI classification
...
public struct RawEvent: Identifiable, Sendable, Hashable {
    ...
    public func closed(at date: Date = Date()) -> RawEvent {
        var copy = self
        copy.endedAt = date
        return copy
    }
    ...
    public func withMetadata(_ json: String?) -> RawEvent {
        var copy = self
