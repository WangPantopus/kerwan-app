...
//   2. Persist non-excluded events to StorageActor (fire-and-forget, background
//      QoS — a storage failure must never stall a capture service).
//   3. Notify AppState of the accepted event count so the UI can display
//      "N events captured today".
//
// Threading
// ─────────
//   `RawEventBuffer` is an actor: `didCapture` is safe to call from any
//   capture service regardless of its own actor isolation.
//   Storage writes are dispatched as unstructured background Tasks so that
//   slow storage I/O does not block the capture pipeline.
...
/// Minimal interface to AppState that CaptureManager and RawEventBuffer need.
///
...
public protocol StorageManaging: AnyActor {
    /// Persist a single raw event. Called on a background task; failures are
...
/// Call sites:
/// ```swift
/// let buffer = RawEventBuffer(exclusionEngine: engine, storage: storage, appState: appState)
...
/// ```
public actor RawEventBuffer: CaptureEventDelegate {
    ...
    private let log = Logger(subsystem: "com.kerwan.app", category: "RawEventBuffer")
    ...
    public func didCapture(_ events: [RawEvent]) async {
        var accepted: [RawEvent] = []
        var excludedCount = 0
        ...
        Task.detached(priority: .background) {
            for event in accepted {
                do {
                    ...
                    logRef.error("Failed to persist RawEvent \(event.id): \(error.localizedDescription)")
                }
            }
        }
    }
