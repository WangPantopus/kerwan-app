...
final class LevenshteinDistanceTests: XCTestCase {
    ...
    func test_identical_strings_return_1() {
        XCTAssertEqual(LevenshteinDistance.similarity("hello", "hello"), 1.0)
    }

    func test_completely_different_strings() {
        // "abc" vs "xyz" — all substitutions, similarity = 1 - 3/3 = 0
        XCTAssertEqual(LevenshteinDistance.similarity("abc", "xyz"), 0.0)
    }

    func test_empty_strings_both_empty_return_1() {
        XCTAssertEqual(LevenshteinDistance.similarity("", ""), 1.0)
    }

    func test_one_empty_returns_0() {
        XCTAssertEqual(LevenshteinDistance.similarity("", "hello"), 0.0)
        XCTAssertEqual(LevenshteinDistance.similarity("hello", ""), 0.0)
    }

    func test_case_insensitive() {
        XCTAssertEqual(LevenshteinDistance.similarity("Jane Smith", "jane smith"), 1.0)
        XCTAssertEqual(LevenshteinDistance.similarity("ALICE", "alice"), 1.0)
    }

    func test_diacritic_insensitive() {
        XCTAssertEqual(LevenshteinDistance.similarity("Café", "cafe"), 1.0)
        XCTAssertEqual(LevenshteinDistance.similarity("José", "Jose"), 1.0)
    }

    func test_high_similarity_typo() {
        // "John Smith" vs "Jon Smith" — 1 deletion, max len 10 → sim = 0.9
        let sim = LevenshteinDistance.similarity("John Smith", "Jon Smith")
        XCTAssertGreaterThan(sim, 0.8)
    }

    func test_length_optimisation_returns_0() {
        // "a" (len 1) vs "aaaaaaaaaa" (len 10) → shorter/longer = 0.1 < 0.4
        XCTAssertEqual(LevenshteinDistance.similarity("a", "aaaaaaaaaa"), 0.0)
    }

    func test_unicode_chinese_names_identical() {
        XCTAssertEqual(LevenshteinDistance.similarity("王芳", "王芳"), 1.0)
    }

    func test_unicode_arabic_names_identical() {
        XCTAssertEqual(LevenshteinDistance.similarity("محمد", "محمد"), 1.0)
    }

    func test_unicode_accented_latin() {
        // "François" vs "Francois" after diacritic strip → identical
        XCTAssertEqual(LevenshteinDistance.similarity("François", "Francois"), 1.0)
    }

    func test_symmetry() {
        let ab = LevenshteinDistance.similarity("Alice Johnson", "Alicia Johnson")
        let ba = LevenshteinDistance.similarity("Alicia Johnson", "Alice Johnson")
        XCTAssertEqual(ab, ba, accuracy: 0.001)
    }

    func test_single_character_difference() {
        // "cat" vs "bat" → 1 substitution, max=3, sim = 2/3 ≈ 0.667
        let sim = LevenshteinDistance.similarity("cat", "bat")
        XCTAssertEqual(sim, 2.0 / 3.0, accuracy: 0.001)
    }
}
