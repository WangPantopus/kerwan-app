...
final class BillingEngineTests: XCTestCase {
    ...
    func test_runDailyClustering_persistsSessionsToStorage() async throws {
        let storage = MockBillingStorage()
        // Seed two events that will cluster into one session.
        await storage.seed(events: [
            makeEvent(id: "x1", start: base, end: base.addingTimeInterval(600)),
            makeEvent(id: "x2", start: base.addingTimeInterval(300), end: base.addingTimeInterval(900))
        ])

        let billingEngine = BillingEngine(
            client:     makeOllamaClient(),
            storage:    storage,
            clustering: SessionClusteringEngine()
        )
        try await billingEngine.runDailyClustering()

        let inserted = await storage.insertedSessions
        XCTAssertEqual(inserted.count, 1, "One cluster → one persisted work session")
        XCTAssertEqual(inserted[0].clientId, "client-A")
    }

    func test_runDailyClustering_emptyEvents_doesNothing() async throws {
        let storage       = MockBillingStorage()
        let billingEngine = BillingEngine(client: makeOllamaClient(), storage: storage)
        try await billingEngine.runDailyClustering()

        let inserted = await storage.insertedSessions
        XCTAssertTrue(inserted.isEmpty, "No events → no sessions inserted")
    }

    func test_runNarrativeGeneration_updatesStorageWithNarrative() async throws {
        MockOllamaURLProtocol.register(path: "api/generate") { _ in
            let ndjson = "{\"response\":\"Worked on Q2 design review.\",\"done\":false}\n" +
                         "{\"response\":\"\\nDesign review — 1.00 hr\",\"done\":true}\n"
        ...
        try await billingEngine.runNarrativeGeneration(for: [session])

        let narratives = await storage.updatedNarratives
        XCTAssertEqual(narratives.count, 1)
        XCTAssertEqual(narratives[0].id, "ws-1")
        XCTAssertNotNil(narratives[0].description)
    }

    func test_runNarrativeGeneration_emptySessionList_doesNothing() async throws {
        let storage       = MockBillingStorage()
        let billingEngine = BillingEngine(client: makeOllamaClient(), storage: storage)
        try await billingEngine.runNarrativeGeneration(for: [])

        let narratives = await storage.updatedNarratives
        XCTAssertTrue(narratives.isEmpty)
    }

    func test_runNarrativeGeneration_ollamaFailure_doesNotThrow() async throws {
        MockOllamaURLProtocol.register(path: "api/generate") { _ in
        ...
        // Should complete without throwing despite the Ollama error.
